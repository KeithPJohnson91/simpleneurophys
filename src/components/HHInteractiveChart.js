...
const PassiveMembraneInteractiveChart = ({_classNameComponents}) => {
    ...
    return (
        <div className={_classNameComponents}>
            ...
            <MySlider key="201" label={"Injected Current"} trackColor={colorChoices['Iinj']} min={-30} max={90} value={Ivalue} onChange={updateIVal}/>
            <MySlider key="202"  label={"Simulation Time"} trackColor={colorChoices['simulation_time']} min={0} max={200} value={Tvalue} onChange={updateTVal} />
            <MySlider key="203"  label={"Stimulus Duration"} trackColor={colorChoices['stimulus_duration']} min={0} max={200} value={Svalue} onChange={updateSVal} />
        </div>
    )
